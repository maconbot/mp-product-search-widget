		...
		if( !class_exists('MPPSW') ) {
			...
			class MPPSW {
				...
				function load_search_form_contents( $settings = array() ) {
					  	...
					  	if ($showcategory) {
					  		...
					  		if ( $globalsearch && is_multisite() ) {
						  		if (!empty($customcategories)) {
						  			$categories = $this->load_global_taxonomies_list('product_category' , $customcategories );
						  		} else {
						  			$categories = $this->load_global_taxonomies_list('product_category');
						  		}
					  		} else {
						  		if (!empty($customcategories)) {
					...
					$output .= '<div class="filter-by-tag-select hide">';
						...
						if ($showtag) {
							...
							if ( $globalsearch && is_multisite() ) {
						  		if (!empty($customtags)) {
						  			$tags = $this->load_global_taxonomies_list('product_tag' , $customtags );
						  		} else {
						  			$tags = $this->load_global_taxonomies_list('product_tag');
						  		}
							} else {
						  		if (!empty($customtags)) {
				...
				function load_search_result_modal( $args = array() ) {
					...
					$output = '<div id="product-search-result-modal-'.$id.'" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="productsearchresultmodal" aria-hidden="true">';
				...
				function load_search_results_listing( $args = array() ) {
					...
					if (!empty($products)) {
						$output = '<form id="mppsw-listing-form" class="mppsw-search-results-listing-form" method="post" action="">';
				...
				function num_product_found( $args = array() ) {
			  	...
			  	function reset_search_box() {
					...
					if (defined('DOING_AJAX') && DOING_AJAX) {
						echo 'success||' . $this->load_search_form_contents($settings);
						exit;
					}
			  	}
				...
				function process_search_query() {
					...
					$success = '<div class="alert alert-success nomargin">';
						...
						$success .= '<div class="btn-group">';
							$success .= '<button href="#" data-toggle="modal" class="btn btn-small'.$settings['btnclass'].'" data-target="#product-search-result-modal-'.$settings['id'].'"><i class="icon-zoom-in'.$settings['iconclass'].'"></i> '.__( 'View Results' , 'mppsw' ).'</button>';
					...
					$success .= apply_filters( 'mppsw_process_search_results_load_modal' , $this->load_search_result_modal( $theinstance ) , $theinstance );
				...
				function load_global_taxonomies_list( $include = 'product_category' , $customlist = '' , $limit = 100 , $order_by = 'name' , $order = 'ASC' ){
				  	...
				  	if (!empty($customlist)) {
				  		$customlist = $wpdb->escape( sanitize_text_field( $customlist ) );
